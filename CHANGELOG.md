...
- 首次同意写入工作台数据目录, 用量、会话、GitHub 和更新检查在未同意时由后端拒绝.
- macOS 授权门禁只拦 Claude, 以及装在系统「应用程序」里的 Cursor; 用户目录安装不再被当成必须授权.
- 会话「退出 Cursor」在温和退出失败后按汉化引擎同样的次数强制结束进程树.
...
- 修复 macOS 从访达或程序坞启动工作台时备份校验报 `无法启动 Cursor 汉化引擎: No such file or directory (os error 2)` 的问题. 图形界面进程不继承 shell `PATH`, 备份校验和安装前门禁改用已探测到的 Node.js 绝对路径, 与汉化主流程保持一致; 启动失败信息同时输出所用解释器路径.
